...
pub fn platform_webview_version() -> Result<String> {
  unsafe {
    ...
    // PATCHED (Immigration-AI-mobile): 원본은 여기서 `bundle.unload()` 를 호출한다.
    // `bundleWithIdentifier:` 가 돌려주는 번들은 CoreFoundation 이 캐싱·소유하는 객체라
    // 우리가 언로드할 대상이 아니다. iOS 에서 이 호출이 과다해제를 일으켜
    // CFRelease 에서 EXC_BREAKPOINT 로 앱이 실행 직후 죽는다 (Wry::init 경로라 무조건 발생).
    // 업스트림 수정 시 벤더 디렉토리째 제거할 것 — docs/adr/0003 참조.
