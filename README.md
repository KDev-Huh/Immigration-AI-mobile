...
아이콘 원본은 `assets/app-icon.png` 하나입니다. 모든 플랫폼 아이콘을 여기서 생성합니다.

```bash
npm run icon
```

**반드시 `npm run android:init` 다음에 실행해야 합니다.** `init`이 만드는 `gen/` 프로젝트에는
Tauri 기본 런처 아이콘이 들어 있고 `gen/`은 커밋하지 않으므로, 이 단계를 건너뛰면 빌드 결과물이
기본 아이콘으로 나갑니다. CI는 이 순서를 강제하고 아이콘이 기본값 그대로면 빌드를 실패시킵니다.

아이콘을 바꾸려면 `assets/app-icon.png`를 교체하고 `npm run icon`을 다시 실행한 뒤 커밋합니다.
1024x1024 정사각 PNG를 권장합니다.
